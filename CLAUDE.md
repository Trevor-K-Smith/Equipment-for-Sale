...
**SSH credentials:** user=`trevorksmith`, password=`Ghostmaniscool1!`
**Firmware:** S100R013
...
```bash
echo -n 'Ghostmaniscool1!' > /tmp/cv_pass.txt
```
...
spawn ssh -o StrictHostKeyChecking=no trevorksmith@$ip
expect "assword:" {
    sleep 1
    send "Ghostmaniscool1!\r"
}
sleep 3
expect "#"
...
sshpass -p 'password' ssh -o StrictHostKeyChecking=no trevorksmith@100.64.0.22 \
  'expect /tmp/cv_run.exp 10.24.1.100 "show disk-groups"'
```
...
### CRITICAL: SSH username is `trevorksmith`, NOT `manage`

The previous `manage` user no longer works. Always use `trevorksmith` for both the SSH username and to authenticate to the Corvault CLI. The old scripts that used `manage@<ip>` will fail with "Password rejected".
